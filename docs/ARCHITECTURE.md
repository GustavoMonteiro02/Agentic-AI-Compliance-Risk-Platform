...
Recommendations should reference retrieved internal policy or regulation summaries. The local RAG path uses Markdown documents enriched with jurisdiction, authority, source URL, document type, effective date, tags, and citation labels. Retrieval combines lexical overlap, phrase matching, domain query expansion, metadata boosts, optional Qdrant vector scores, and an explainable metadata cross-signal reranker with score breakdowns, matched terms, rank reasons, citation quality, and evidence grade. The API exposes this path through `GET /requirements/search` with metadata filters for auditor-facing discovery. Qdrant can be populated through `make ingest-qdrant` and stores the same citation-rich payload used by the local fallback.
...
Official legal-corpus ingestion is organized through `data/legal_sources_manifest.json`. Article-level Markdown files under `data/legal_sources/` can carry locators, source URLs, effective dates, tags, and content hashes for citation-grade retrieval.
